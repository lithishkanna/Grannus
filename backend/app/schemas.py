...
# ---------------------------------------------------------------------------
# LOW priority — self-care guidance (Gemini generated, safety-constrained)
# ---------------------------------------------------------------------------

class HomeRemedyCareStep(BaseModel):
    """A single self-care action step."""
    step: str = Field(description="The care action in plain language, e.g. 'Rest and drink plenty of fluids'.")
    translated_step: Optional[str] = Field(default=None, description="Step translated into patient language.")


class HomeRemedyGuidance(BaseModel):
    """
    Self-care guidance for LOW priority cases.
    Generated by Gemini with strict safety constraints:
    - No disease diagnosis
    - No medication names or dosages
    - No 'cure' language
    - Must include seek-doctor triggers
    """
    care_steps: List[HomeRemedyCareStep] = Field(
        default_factory=list,
        description="Ordered list of general self-care actions appropriate for the reported symptoms.",
    )
    monitoring_signs: List[str] = Field(
        default_factory=list,
        description="Symptoms or changes to watch for that would indicate worsening condition.",
    )
    seek_doctor_if: List[str] = Field(
        default_factory=list,
        description="Specific conditions under which the patient should immediately seek medical attention.",
    )
    translated_care_steps: Optional[List[str]] = Field(
        default=None,
        description="Care steps translated into the patient's language.",
    )
    translated_seek_doctor_if: Optional[List[str]] = Field(
        default=None,
        description="Seek-doctor triggers translated into patient's language.",
    )
    disclaimer: str = Field(
        ...
        description="Mandatory safety disclaimer — always included.",
    )
    language: Optional[str] = Field(default=None, description="BCP-47 code for patient language used in translations.")


# ---------------------------------------------------------------------------
# MEDIUM / HIGH priority — summary translated for doctor's preferred language
# ---------------------------------------------------------------------------

class DoctorTranslatedSummary(BaseModel):
    """
    Key clinical summary fields translated into the doctor's preferred language.
    Used when the doctor's interface language differs from English.
    """
    chief_complaint: Optional[str] = Field(default=None, description="Chief complaint translated to doctor language.")
    symptoms_summary: Optional[str] = Field(
        default=None,
        description="Narrative summary of all non-negated symptoms translated to doctor language.",
    )
...
class PipelineResult(BaseModel):
    ...
    # ---- LOW priority: self-care guidance ----
    home_remedy_guidance: Optional[HomeRemedyGuidance] = Field(
        default=None,
        description="Self-care guidance generated by Gemini for LOW priority cases only. Null for MEDIUM/HIGH.",
    )

    # ---- MEDIUM / HIGH priority: summary for doctor ----
    doctor_translated_summary: Optional[DoctorTranslatedSummary] = Field(
        default=None,
        description="Clinical summary translated into the doctor's preferred language. Null for LOW priority or when doctor language is English.",
    )
